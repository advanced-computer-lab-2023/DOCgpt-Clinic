...
export const getSessionPrice = async (
  req: Request,
  res: Response,
  username: string
) => {};
...
export const searchDoctors = async (
  ...
  try {
    const { name, speciality } = req.query as {
      name: string;
      speciality: string;
    };
...
export const selectDoctors = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
...
export const logout = async (req: Request, res: Response) => {
  try {
    ...
    jwt.verify(
      ...
      async (err: jwt.JsonWebTokenError | null, user: any) => {
        if (err) {
...
export const verifyTokenPatient = (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  ...
  try {
    ...
    if (uploadedFiles) {
      ...
      if (section == "MedicationList" && subsection == "CurrentMedications") {
        healthRecord.MedicationList?.CurrentMedications?.Prescriptions.push(
          path
        );
      } else if (
        section == "MedicationList" &&
    ...
    if (section == "MedicationList" && subsection == "CurrentMedications") {
      if (healthRecord.MedicationList?.CurrentMedications) {
        const updatedPrescriptions =
          healthRecord.MedicationList?.CurrentMedications?.Prescriptions.filter(
            (prescription) => prescription !== path
          );
      ...
      if (healthRecord.MedicationList?.PastMedications) {
        const updatedPrescriptions =
          healthRecord.MedicationList?.PastMedications?.Prescriptions.filter(
            (prescription) => prescription !== path
          );
        if (updatedPrescriptions !== undefined) {
      ...
      if (healthRecord.Laboratory?.BloodTests) {
        const updated = healthRecord.Laboratory?.BloodTests?.filter(
          (prescription) => prescription !== path
        );
      ...
      if (healthRecord.Laboratory?.XRays) {
        const updated = healthRecord.Laboratory?.XRays?.filter(
          (prescription) => prescription !== path
        );
        if (updated !== undefined) {
      ...
      if (healthRecord.Laboratory?.Other) {
        const updated = healthRecord.Laboratory?.Other?.filter(
          (prescription) => prescription !== path
        );
        if (updated !== undefined) {
      ...
      if (healthRecord.GeneralImages) {
        const updatedPrescriptions = healthRecord.GeneralImages.filter(
          (prescription) => prescription !== path
        );
        if (updatedPrescriptions !== undefined) {
