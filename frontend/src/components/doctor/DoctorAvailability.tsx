  ...
  const handleAdd = () => {
    ...
    console.log(timeslots);
        ...
        for (const ts of timeslots) {
          if (new Date(ts.date).toISOString() === combinedDateTimeString) {
            // Date already exists in the timeslots array
            setOpenAlert(true)
            return; // Break out of the loop since you've found a match
          }
        }
      // Date is unique, add it to the array
                    ...
                    {selectedDates.map((date, index) => (
                      <div key={index}>
                        ...
                        <span style={{ fontWeight: "bold" }}>
                          {new Date(date).getHours() === 14
                            ? new Date(date).getHours() - 2
                            : new Date(date).getHours() === 13 
                            ? new Date(date).getHours() - 2
                          ...
                          {new Date(date).getHours() === 0? "PM"
                          :new Date(date).getHours() === 1? "PM"
                          :new Date(date).getHours() >= 14 ? "PM" : "AM"}
                        </span>
    ...
    {timeslots.map((timeslot, index) => (
      ...
      >
        <MuiAlert severity="error" sx={{ width: '100%', fontSize: '1.5rem' }}>
          You have already added this Time slot, please choose another one 
        </MuiAlert>
      </Snackbar>
      {/* Snackbar */}
      <El7a2niInfo />
