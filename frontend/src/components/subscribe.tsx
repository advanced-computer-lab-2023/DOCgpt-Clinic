...
const UnsubscribePackageForMember: React.FC = () => {
  const [familyMembersAndPackages, setFamilyMembersAndPackages] = useState<FamilyMemberPackage[] | null>(null);
  const [selectedPackage, setSelectedPackage] = useState<FamilyMemberPackage | null>(null);
  const [isConfirmationDialogOpen, setConfirmationDialogOpen] = useState<boolean>(false);

  const handleUnsubscribe = async (familyMember: FamilyMemberPackage | null) => {
    try {
      if (!familyMember) {
      ...
      const response = await axios.patch('/routes/cancelSubscriptionfam', {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const token = localStorage.getItem('authToken');
        const response: AxiosResponse<{ familyMembersAndPackages: FamilyMemberPackage[] }> = await axios.get('/routes/viewFamilyMembersAndPackages', {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  return (
    <Container>
      ...
      <Grid container spacing={2}>
        {familyMembersAndPackages && familyMembersAndPackages.map((item: FamilyMemberPackage, index: number) => (
          <Grid item key={index} xs={12} sm={6} md={4} lg={3}>
            <Paper style={{ padding: '20px' }}>
              <Typography variant="h6">{item.familyMemberName
}</Typography>
      ...
      <Dialog open={isConfirmationDialogOpen} onClose={() => setConfirmationDialogOpen(false)}>
        ...
        <DialogContent>
          <Typography>
            Are you sure you want to unsubscribe from the package for {selectedPackage?.familyMemberName
}?
          </Typography>
        </DialogContent>
        <DialogActions>
