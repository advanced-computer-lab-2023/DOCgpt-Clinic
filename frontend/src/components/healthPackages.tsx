import { Button, Card, Container, Grid, Typography, Dialog, DialogContent } from "@mui/material";
...
const HealthPackages = () => {
  ...
  const handleDeletePackage = async (packageName: string) => {
    if (window.confirm(`Are you sure you want to delete ${packageName}?`)) {
      try {
        await axios.delete(`/routes/patient/deleteHealthPackage/${packageName}`);
        refreshHealthPackages();
      } catch (error) {
  ...
  return (
    ...
    <Container>
      {healthPackages.map((healthPackage) => (
        <Card style={{ padding: '20px', margin: '20px' }} key={healthPackage.name}>
          <Grid container spacing={2}>
            ...
            <Grid item xs={2} style={{ display: 'flex', alignItems: 'center' }}>
              ...
              <Button variant="contained" onClick={() => handleOpenUpdateDialog(healthPackage)}>
                Update
              </Button>
            </Grid>
            <Grid item xs={2} style={{ display: 'flex', alignItems: 'center' }}>

              <Button variant="contained" color="error" onClick={() => handleOpenDeleteDialog(healthPackage)}>
                Delete
              </Button>
            </Grid>
          </Grid>
