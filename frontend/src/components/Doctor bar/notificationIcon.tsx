...
import { formatTimeDifference } from '../patientBar/utils'; // Adjust the path accordingly
...
const CustomizedBadges = () => {
  ...
  const fetchNotificationsCount = async () => {
    try {
      ...
      const response = await axios.get('/routes/notifications/countD', {
        headers: {
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      try {
        const token = localStorage.getItem('authToken');
        const response = await axios.get('/routes/notifications/doctor', {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
